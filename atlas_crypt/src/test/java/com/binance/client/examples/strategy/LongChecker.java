...
public class LongChecker extends Checker {
    ...
    public LongChecker(String symbol
...
//            }
//        }else {
//            if (!(trendCandlestick1.getHigh().compareTo(trendCandlestick2.getHigh()) < 0 && trendCandlestick0.getHigh().compareTo(trendCandlestick1.getHigh()) < 0)) {
//                return true;
//            }
//        }
//        return false;
...
//
//                    //回踩
//                    && opCandlestick1.getLow().doubleValue() <= resistancePrices.get(i)
//
//                    && opCandlestick1.getClose().doubleValue() >= resistancePrices.get(i)
//
//            ) {
...
//            int level = i + 1;
//
//            BigDecimal upSpace = new BigDecimal((resistancePrice1 - opCandlestick0.getClose().doubleValue()) / opCandlestick0.getClose().doubleValue());
//
...
//                && opCandlestick0.getClose().doubleValue() >= MathUtil.max(opCandlestick1.getHigh(), opCandlestick2.getHigh()).doubleValue()
//        ) {
//            return CheckResponse.builder()
//                    .success(true)
//                    .periodType(opPeriodType)
...
//
//                && opCandlestick0.getChangeRate().compareTo(new BigDecimal("0.008")) > 0
//
...
//            if (tiZiCandlestick.getShiTiRate().compareTo(new BigDecimal("0.02")) > 0) {
//                if (opCandlestick0.getShiTiRate().compareTo(BigDecimal.ZERO) > 0
//                        && opCandlestick0.getClose().compareTo(tiZiCandlestick.getLow()) > 0
//
//                        && (opCandlestick0.getClose().compareTo(MathUtil.max(opCandlestick1.getShiTiMax(), opCandlestick2.getShiTiMax())) > 0
...
//        List<Candlestick> tiZiPoints = LongCheckerUtils.getTiZiList(lastCandlesticks, trendPeriodType);
//
...
//
//            //底上移
//            if (LongCheckerUtils.isBackCrossTiZiMin(tiZiTrade, lastCandlesticks)) {
//                continue;
//            }
//
//            //区间收盘未突破梯子High
//            if (LongCheckerUtils.isOverTiZiHigh(tiZiTrade, lastCandlesticks)) {
//                continue;
//            }
//
//            //突破 梯子high
//            BigDecimal tiziHigh = tiZiTrade.getHigh();
//            if (opTrade1.getClose().compareTo(tiziHigh) <= 0
//                    && opTrade0.getClose().compareTo(tiziHigh) >= 0
//
//                    && opTrade0.getClose().compareTo(opTrade1.getShiTiMax()) > 0
//                    && opTrade0.getHigh().compareTo(opTrade1.getHigh()) > 0
//
//                    && opTrade0.getChangeRate().compareTo(trendPeriodType.getCrossTiZiRate()) > 0
//
//            ) {
//
//                return CheckResponse.builder()
...
//    }
    ...
    public CheckResponse checkCrossKeyPressureHighSignal(List<PeriodTypeEnum> trendPeriodTypes, PeriodTypeEnum opPeriodType) {
        ...
        Map<PeriodTypeEnum, List<Candlestick>> keyPressureMap = LongCheckerUtils.getKeyPressureMap(getLastCandlesticks(trendPeriodType, 5), trendPeriodType, getLastCandlesticks(opPeriodType, 50), opPeriodType);
        ...
        for (Map.Entry<PeriodTypeEnum, List<Candlestick>> entry: keyPressureMap.entrySet()) {
            ...
            for (Candlestick tiZiCandlestick: keyPressureList) {
                if (opTrade0.getLow().compareTo(tiZiCandlestick.getHigh()) <= 0
                        && opTrade0.getClose().compareTo(tiZiCandlestick.getHigh()) > 0
//                        && opTrade0.getChangeRate().compareTo(opPeriodType.getCrossTiZiHighRate()) > 0
                ) {
                    return CheckResponse.builder()
                            .success(false)
                            .opPeriodType(opPeriodType)
                            .trendPeriodType(trendPeriodType)
                            .message("突破关键阻力位High(" + ":" + entry.getKey() + ":" + tiZiCandlestick.getOpenTimeStr() + ":" + tiZiCandlestick.getHigh() + ":" + opTrade0.getChangeRate() + ")")
                            .build();
                }
            }
        }
...
//
//                    && opCandlestick0.getClose().compareTo(opCandlestick1.getShiTiMax()) > 0
//                    && opCandlestick0.getHigh().compareTo(opCandlestick1.getHigh()) > 0
//            ) {
//                return CheckResponse.builder()
//                        .success(true)
//                        .periodType(trendPeriodType)
//                        .message("震荡上移(梯子区间)(" + "[" + tiZiCandlestick.getClose()  + "]" + tiZiCandlestick.getOpenTimeStr() + ")")
//                        .build();
//            }
//        }
//        return CheckResponse.builder()
