...
public final class LongCheckerUtils {

    /**
     * 梯子特征：
     * 1、较大比率实体，且后一个周期未连续突破：能量蓄积且未释放
     */
    public static List<Candlestick> getKeyPressureMap(List<Candlestick> trades, PeriodTypeEnum periodType) {
        ...
        for (int i = trades.size() - 3; i > 1; i--) {
            ...
            if (tmpTrade0.getChangeRate().compareTo(periodType.getCrossTiZiHighRate()) > 0
     ...
     * 1、较大比率实体，且后一个周期未连续突破：能量蓄积且未释放
     */
    public static List<Candlestick> getTiZiListWithNoMerge(List<Candlestick> trades, PeriodTypeEnum periodType) {
    ...
    public static boolean isBackCrossTiZiMin(Candlestick tiZiTrade, List<Candlestick> opTrades) {
        for (int i = 0; i < opTrades.size(); i++) {
            ...
            if (candlestick.getLow().compareTo(tiZiTrade.getOpen()) <= 0) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isOverTiZiHigh(Candlestick tiZiTrade, List<Candlestick> opTrades) {
        for (int i = 0; i < opTrades.size(); i++) {
            Candlestick candlestick = opTrades.get(i);
            if (candlestick.getOpenTime() <= tiZiTrade.getOpenTime()) {
                continue;
            }
            if (candlestick.getClose().compareTo(tiZiTrade.getHigh()) > 0) {
                return true;
            }
        }
        return false;
