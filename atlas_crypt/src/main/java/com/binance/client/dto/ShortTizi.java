...
public class ShortTizi {
    ...
    public ShortTizi(String symbol, List<Candlestick> trades, PeriodTypeEnum periodTypeEnum, BigDecimal tprice) {
        this.symbol = symbol;
        this.trades = trades;
    ...
    public BigDecimal getOpen() {
        return this.trades.stream().map(x -> x.getOpen()).max(Comparator.comparing(BigDecimal::doubleValue)).get();
    }
    ...
    public BigDecimal getClose() {
        return this.trades.stream().map(x -> x.getClose()).min(Comparator.comparing(BigDecimal::doubleValue)).get();
    }
