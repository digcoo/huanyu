...
/**
 * 策略3：1H 金叉波段突破或死叉点突破，父级 4H 均价之上。
 */
public final class HourMaCrossBreakTools {
    ...
    static Hit findGoldenBandTopHit(List<LongCandlestickMA> bars) {
        ...
        return new Hit(MaCrossPointCore.CrossKind.GOLDEN, signalBar, prevBar, crossBar,
                breakLine, findByTime(bars, band.getBandHighBar()));
    }
    ...
    static Hit findDeathCrossPointHit(List<LongCandlestickMA> bars) {
        ...
        return new Hit(MaCrossPointCore.CrossKind.DEATH, signalBar, prevBar, bars.get(crossIdx), breakLine, null);
    }
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                "%s,strategyTag=%s,period=1h,parent=4h,cross=%s,target=%s,sigTime=%s,sigClose=%.8f,breakLine=%.8f,crossTime=%s",
                death ? "MA死叉点突破" : "MA金叉波段突破",
                death ? "MDB" : "MGB",
