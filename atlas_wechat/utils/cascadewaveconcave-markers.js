...
function shouldShow(id, period) {
  if (adapter.normalizeStrategyId(id) !== 'cascadewaveconcave') return false;
...
function fetchOverlayForItem(item, id, period) {
  ...
  return stockApi.fetchCascadewaveconcaveMarkers(item.code, period)
    .then(function (vo) {
      if (vo) return overlayFromVo(vo, item);
      return stockApi.fetchSummary(item.code, 'cascadewaveconcave').then(function () {
        return overlayFromItem(item);
      });
    })
    .catch(function () {
...
function enrichItemsWithCascadewaveconcaveMarkers(items, strategyId, period) {
...
function shouldShowCascadewaveconcaveMarkers(id, period) {
  return shouldShow(id, period);
}

module.exports = {
  shouldShowCascadewaveconcaveMarkers: shouldShowCascadewaveconcaveMarkers,
  resolveOverlayForItem: resolveOverlayForItem,
  resolveBarMarkersForItem: resolveBarMarkersForItem,
  enrichItemsWithCascadewaveconcaveMarkers: enrichItemsWithCascadewaveconcaveMarkers,
