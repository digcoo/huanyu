...
function shouldShow(id, period) {
  if (adapter.normalizeStrategyId(id) !== 'cascadewaveconvex') return false;
...
function fetchOverlayForItem(item, id, period) {
  ...
  return stockApi.fetchCascadewaveconvexMarkers(item.code, period)
    .then(function (vo) {
      if (vo) return overlayFromVo(vo, item);
      return stockApi.fetchSummary(item.code, 'cascadewaveconvex').then(function () {
        return overlayFromItem(item);
      });
    })
    .catch(function () {
...
function enrichItemsWithCascadewaveconvexMarkers(items, strategyId, period) {
...
function shouldShowCascadewaveconvexMarkers(id, period) {
  return shouldShow(id, period);
}

module.exports = {
  shouldShowCascadewaveconvexMarkers: shouldShowCascadewaveconvexMarkers,
  resolveOverlayForItem: resolveOverlayForItem,
  resolveBarMarkersForItem: resolveBarMarkersForItem,
  enrichItemsWithCascadewaveconvexMarkers: enrichItemsWithCascadewaveconvexMarkers,
