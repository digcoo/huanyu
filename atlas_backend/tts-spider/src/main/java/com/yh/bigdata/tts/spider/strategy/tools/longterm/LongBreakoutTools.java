...
/**
 * 长线策略 · 前 N 自然年月K 基准（A1 年内局部新高强K + A2 最近强K）+ 本年月K 突破
 */
public final class LongBreakoutTools {

    private static final double HIGH_EPS = 1e-6;
    private static final int FETCH_BARS = 80;

    private LongBreakoutTools() {
    }
    ...
    public static Hit findHit(StockBase stock, LongStrategyParams params) {
        LongStrategyParams p = params != null ? params : LongStrategyParams.defaults();
        List<Trade> allBars = RealtimeStockCache.getLastTrades(stock, PeriodTypeEnum.MONTH, FETCH_BARS);
    ...
    /** A1：按自然年分组，强K 且 high 严格高于该年已出现最高价 → 取时间上最后一根 */
    static Trade findLastLocalHighStrongRef(List<Trade> priorBars, double minStrongPct) {
        Trade ref = null;
        String currentYear = null;
        double yearHighSoFar = -Double.MAX_VALUE;

        for (Trade bar : priorBars) {
            String year = BreakoutBucketTools.yearKey(bar);
            if (year == null || bar.getHigh() == null) {
                continue;
            }
            if (!year.equals(currentYear)) {
                currentYear = year;
                yearHighSoFar = -Double.MAX_VALUE;
            }
            double high = bar.getHigh();
            if (BreakoutBarTools.isStrongBar(bar, minStrongPct) && high > yearHighSoFar + HIGH_EPS) {
                ref = bar;
            }
            yearHighSoFar = Math.max(yearHighSoFar, high);
        }
        return ref;
    }
    ...
    static Trade findSignalBar(StockBase stock, List<Trade> allBars, List<Trade> signalBars, Trade refBar,
        ...
        if (requireCurrentBreakout) {
            ...
            return BreakoutSignalTools.isSignalCandidate(stock, currentBar, allBars, refLow, refHigh,
                    minStrongPct, BreakoutPositionContextTools.MacroTier.LONG) ? currentBar : null;
        }
        ...
        for (Trade bar : signalBars) {
            if (BreakoutSignalTools.isSignalCandidate(stock, bar, allBars, refLow, refHigh, minStrongPct,
                    BreakoutPositionContextTools.MacroTier.LONG)) {
                lastMatch = bar;
            }
        }
        return lastMatch;
