...
import com.yh.bigdata.tts.spider.strategy.tools.macrosspoint.MaYangPierceEvaluator;
...
@Slf4j
@Component
public class MaYangPierceStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_YANG_PIERCE;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            ...
            MaCrossPointEvaluator.MaCrossPointEvaluation eval =
                    MaYangPierceEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private MaCrossPointStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaYangPierce() == null) {
            return MaCrossPointStrategyParams.defaults();
        }
        return MaCrossPointStrategyParams.merge(queryContextParam.getMaYangPierce());
    }
