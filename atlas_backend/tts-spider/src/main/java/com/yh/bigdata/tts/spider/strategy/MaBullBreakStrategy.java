...
@Slf4j
@Component
public class MaBullBreakStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_BULL_BREAK;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            ...
            MaCrossPointEvaluator.MaCrossPointEvaluation eval =
                    MaTrendBreakEvaluator.evaluate(stockBase, checkResult, params, true);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private MaCrossPointStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaBullBreak() == null) {
            return MaCrossPointStrategyParams.defaults();
        }
        return MaCrossPointStrategyParams.merge(queryContextParam.getMaBullBreak());
    }
