...
/**
 * 周凹凸突破：周 MACD&gt;0；周 K 凹/凸边沿破波段 High 或凸边沿回踩（信号限末根周 K）。
 */
public final class WeekWaveCcBreakoutTools {

    private static final double EPS = 1e-6;
    private static final int WEEK_MACD_LOOKBACK = 40;

    private WeekWaveCcBreakoutTools() {
    }
    ...
    public static Hit findHit(StockBase stock, WeekWaveCcBreakoutStrategyParams params) {
        WeekWaveCcBreakoutStrategyParams p = params != null ? params : WeekWaveCcBreakoutStrategyParams.defaults();
        if (stock == null || !passesWeekMacdGate(stock)) {
            return null;
        }
        int lookback = Math.max(p.getLookbackBars(), 10);
        int fetchBars = Math.max(lookback + 40, lookback + 2);
        List<Trade> trades = RealtimeStockCache.getLastTrades(stock, PeriodTypeEnum.WEEK, fetchBars);
    ...
    static Hit resolveHitOnBars(List<Trade> trades, WeekWaveCcBreakoutStrategyParams params) {
        WeekWaveCcBreakoutStrategyParams p = params != null ? params : WeekWaveCcBreakoutStrategyParams.defaults();
        if (CollectionUtils.isEmpty(trades) || trades.size() < 3) {
            return null;
        }
    ...
    public static boolean passesWeekMacdGate(StockBase stock) {
        return lastMacd(stock) > 0;
    }

    static double lastMacd(StockBase stock) {
        List<Trade> bars = RealtimeStockCache.getLastTrades(stock, PeriodTypeEnum.WEEK, WEEK_MACD_LOOKBACK);
    ...
    public static boolean passesOptionalGates(StockBase stock, CheckResult checkResult,
                                              WeekWaveCcBreakoutStrategyParams params, Hit hit) {
        WeekWaveCcBreakoutStrategyParams p = params != null ? params : WeekWaveCcBreakoutStrategyParams.defaults();
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                "周凹凸突破,strategyTag=WWCCB,period=week,hitMode=%s,"
    ...
    public static String buildTrendMessage(Hit hit) {
        ...
        return String.format("[WWCCB]周凹凸突破|周MACD>0,%s|breakLine=%.2f",
    ...
    private static void appendMessage(CheckResult checkResult, String msg) {
        if (checkResult != null) {
            checkResult.addTrendPeriod(PeriodTypeEnum.WEEK, "[WWCCB]" + msg);
        }
    }
}
