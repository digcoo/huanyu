...
import com.yh.bigdata.tts.spider.strategy.tools.cascadewaveconcaveday.CascadeWaveConcaveDayEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.cascadewaveconcaveday.CascadeWaveConcaveDayScoreCalculator;
...
@Slf4j
@Component
public class CascadeWaveConcaveDayStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.CASCADE_WAVE_CONCAVE_DAY;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            CascadeWaveConcaveDayStrategyParams params = resolveParams(queryContextParam);
            CascadeWaveConcaveDayEvaluator.CascadeWaveConcaveDayEvaluation eval =
                    CascadeWaveConcaveDayEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            ...
            checkResult.setOpPeriodType(CascadeWaveConcaveDayScoreCalculator.primaryPeriod(eval));
    ...
    private CascadeWaveConcaveDayStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getCascadeWaveConcaveDay() == null) {
            return CascadeWaveConcaveDayStrategyParams.defaults();
        }
        return CascadeWaveConcaveDayStrategyParams.merge(queryContextParam.getCascadeWaveConcaveDay());
    }
