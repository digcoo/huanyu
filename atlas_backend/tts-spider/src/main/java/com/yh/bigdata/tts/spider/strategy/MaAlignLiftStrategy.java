...
import com.yh.bigdata.tts.spider.strategy.tools.maalignlift.MaAlignLiftEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.maalignlift.MaAlignLiftTools;
...
@Slf4j
@Component
public class MaAlignLiftStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_ALIGN_LIFT;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            MaAlignLiftStrategyParams params = resolveParams(queryContextParam);
            MaAlignLiftEvaluator.MaAlignLiftEvaluation eval =
                    MaAlignLiftEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : MaAlignLiftTools.resolvePeriod(params.getTier());
    ...
    private MaAlignLiftStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaAlignLift() == null) {
            return MaAlignLiftStrategyParams.defaults();
        }
        return MaAlignLiftStrategyParams.merge(queryContextParam.getMaAlignLift());
    }
