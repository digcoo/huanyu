...
import com.yh.bigdata.tts.spider.strategy.tools.trendma.TrendMaEvaluator;
...
@Slf4j
@Component
public class TrendMaStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.TREND_MA;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            TrendMaStrategyParams params = resolveParams(queryContextParam);
            TrendMaEvaluator.TrendMaEvaluation eval =
                    TrendMaEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private TrendMaStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getTrendMa() == null) {
            return TrendMaStrategyParams.defaults();
        }
        return TrendMaStrategyParams.merge(queryContextParam.getTrendMa());
    }
