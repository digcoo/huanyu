...
import com.yh.bigdata.tts.spider.strategy.tools.macrosspoint.MaGoldenHighBreakEvaluator;
...
@Slf4j
@Component
public class MaGoldenHighBreakStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_GOLDEN_HIGH_BREAK;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            ...
            MaCrossPointEvaluator.MaCrossPointEvaluation eval =
                    MaGoldenHighBreakEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private MaCrossPointStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaGoldenHighBreak() == null) {
            return MaCrossPointStrategyParams.defaults();
        }
        return MaCrossPointStrategyParams.merge(queryContextParam.getMaGoldenHighBreak());
    }
