...
import com.yh.bigdata.tts.spider.strategy.tools.mabearbreak.MaBearBreakEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.mabearbreak.MaBearBreakTools;
...
@Slf4j
@Component
public class MaBearBreakStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_BEAR_BREAK;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            MaBearBreakStrategyParams params = resolveParams(queryContextParam);
            MaBearBreakEvaluator.MaBearBreakEvaluation eval =
                    MaBearBreakEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : MaBearBreakTools.resolvePeriod(params.getTier());
    ...
    private MaBearBreakStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaBearBreak() == null) {
            return MaBearBreakStrategyParams.defaults();
        }
        return MaBearBreakStrategyParams.merge(queryContextParam.getMaBearBreak());
    }
