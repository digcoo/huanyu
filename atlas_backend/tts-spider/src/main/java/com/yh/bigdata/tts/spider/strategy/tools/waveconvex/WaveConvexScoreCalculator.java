...
public final class WaveConvexScoreCalculator {

    private WaveConvexScoreCalculator() {
    }

    public static int computeScore(WaveConvexEvaluator.WaveConvexEvaluation eval) {
        int score = 32;
        int hits = 0;
    ...
    public static WaveShapeTools.TierHit primaryHit(WaveConvexEvaluator.WaveConvexEvaluation eval) {
        if (eval.getMonthHit() != null) {
    ...
    public static PeriodTypeEnum primaryPeriod(WaveConvexEvaluator.WaveConvexEvaluation eval) {
        WaveShapeTools.TierHit hit = primaryHit(eval);
        return hit != null ? hit.getSignalTier() : PeriodTypeEnum.DAY;
    }

    public static String buildTrendMessage(WaveConvexEvaluator.WaveConvexEvaluation eval,
                                           WaveConvexStrategyParams params) {
        StringBuilder sb = new StringBuilder("[WAVECONVEX]凸波段突破|日周月末阳底上方");
    ...
    public static String buildSignalMessage(WaveShapeTools.TierHit hit) {
        ...
        return String.format(
                "凸波段突破,bandShape=CONVEX,breakPath=%s,signalTier=%s,refPeriod=%s,"
    ...
    private static void appendTierTrendLabel(StringBuilder sb, WaveShapeTools.TierHit hit) {
        if (hit != null) {
            sb.append('|').append(periodLabel(hit.getSignalTier())).append("凸波段同档突破");
        }
    }
