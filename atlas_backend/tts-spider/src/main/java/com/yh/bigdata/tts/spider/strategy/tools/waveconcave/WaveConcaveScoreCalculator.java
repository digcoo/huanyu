...
public final class WaveConcaveScoreCalculator {

    private WaveConcaveScoreCalculator() {
    }

    public static int computeScore(WaveConcaveEvaluator.WaveConcaveEvaluation eval) {
        int score = 32;
        int hits = 0;
    ...
    public static WaveShapeTools.TierHit primaryHit(WaveConcaveEvaluator.WaveConcaveEvaluation eval) {
        if (eval.getMonthHit() != null) {
    ...
    public static PeriodTypeEnum primaryPeriod(WaveConcaveEvaluator.WaveConcaveEvaluation eval) {
        WaveShapeTools.TierHit hit = primaryHit(eval);
        return hit != null ? hit.getSignalTier() : PeriodTypeEnum.DAY;
    }

    public static String buildTrendMessage(WaveConcaveEvaluator.WaveConcaveEvaluation eval,
                                           WaveConcaveStrategyParams params) {
        StringBuilder sb = new StringBuilder("[WAVECONCAVE]凹波段突破|日周月末阳底上方");
    ...
    public static String buildSignalMessage(WaveShapeTools.TierHit hit) {
        ...
        return String.format(
                "凹波段突破,bandShape=CONCAVE,breakPath=%s,signalTier=%s,refPeriod=%s,"
    ...
    private static void appendTierTrendLabel(StringBuilder sb, WaveShapeTools.TierHit hit) {
        if (hit != null) {
            sb.append('|').append(periodLabel(hit.getSignalTier())).append("凹波段同档突破");
        }
    }
