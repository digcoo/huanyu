...
import com.yh.bigdata.tts.spider.strategy.tools.trendretesthigh.TrendRetestHighEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.trendretesthigh.TrendRetestHighTools;
...
@Slf4j
@Component
public class TrendRetestHighStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.TREND_RETEST_HIGH;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            TrendRetestHighStrategyParams params = resolveParams(queryContextParam);
            TrendRetestHighEvaluator.TrendRetestHighEvaluation eval =
                    TrendRetestHighEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : TrendRetestHighTools.resolvePeriod(params.getTier());
    ...
    private TrendRetestHighStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getTrendRetestHigh() == null) {
            return TrendRetestHighStrategyParams.defaults();
        }
        return TrendRetestHighStrategyParams.merge(queryContextParam.getTrendRetestHigh());
    }
