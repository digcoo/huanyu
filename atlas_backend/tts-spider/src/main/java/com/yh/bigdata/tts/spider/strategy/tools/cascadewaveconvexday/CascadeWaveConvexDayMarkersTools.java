...
public final class CascadeWaveConvexDayMarkersTools {
    ...
    private CascadeWaveConvexDayMarkersTools() {
    }
    ...
    public static CascadeWaveBreakoutTools.TierHit findHitForPeriod(StockBase stock, PeriodTypeEnum period,
        ...
        switch (period) {
            case WEEK:
                ...
                return CascadeWaveBreakoutTools.findDayEdgeTierHit(stock, PeriodTypeEnum.WEEK, p.getLookbackWeek(),
                        WaveShapeTools.BandShape.CONVEX, p.isEnablePrevBandBreak());
            case MONTH:
                if (!p.isEnableMonth()) {
                    return null;
                }
                return CascadeWaveBreakoutTools.findDayEdgeTierHit(stock, PeriodTypeEnum.MONTH, p.getLookbackMonth(),
                        WaveShapeTools.BandShape.CONVEX, p.isEnablePrevBandBreak());
            case DAY:
            default:
                if (!p.isEnableDay()) {
                ...
                return CascadeWaveBreakoutTools.findDayEdgeTierHit(stock, PeriodTypeEnum.DAY, p.getLookbackDay(),
                        WaveShapeTools.BandShape.CONVEX, p.isEnablePrevBandBreak());
        }
    }
