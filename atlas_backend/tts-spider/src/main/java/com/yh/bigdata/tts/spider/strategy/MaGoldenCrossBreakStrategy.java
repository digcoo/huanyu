...
import com.yh.bigdata.tts.spider.strategy.tools.macrosspoint.MaGoldenCrossBreakEvaluator;
...
@Slf4j
@Component
public class MaGoldenCrossBreakStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_GOLDEN_CROSS_BREAK;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            ...
            MaCrossPointEvaluator.MaCrossPointEvaluation eval =
                    MaGoldenCrossBreakEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private MaCrossPointStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaGoldenCrossBreak() == null) {
            return MaCrossPointStrategyParams.defaults();
        }
        return MaCrossPointStrategyParams.merge(queryContextParam.getMaGoldenCrossBreak());
    }
