...
import com.yh.bigdata.tts.spider.strategy.tools.macdcrosstier.MacdCrossTierEvaluator;
...
/**
 * 同档 MACD 交叉突破（macdcrosstier）：日/周/月单档边沿破 MACD 交叉 K high。
 */
@Slf4j
@Component
public class MacdCrossTierStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MACD_CROSS_TIER;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            MacdCrossTierStrategyParams params = resolveParams(queryContextParam);
            MacdCrossTierEvaluator.MacdCrossTierEvaluation eval =
                    MacdCrossTierEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private MacdCrossTierStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMacdCrossTier() == null) {
            return MacdCrossTierStrategyParams.defaults();
        }
        return MacdCrossTierStrategyParams.merge(queryContextParam.getMacdCrossTier());
    }
