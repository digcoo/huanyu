...
/**
 * 死叉突破 · 最近 MACD 绿柱死叉 K（ref）
 */
public final class Dc2StructureTools {

    private Dc2StructureTools() {
    }

    public static Trade findLatestDeathCrossBar(List<Trade> trades, int lookback) {
        ...
        for (int i = points.size() - 1; i >= start; i--) {
            if (points.get(i).isIfGreenGoldCross()) {
                return trades.get(i);
            }
        }
        return null;
