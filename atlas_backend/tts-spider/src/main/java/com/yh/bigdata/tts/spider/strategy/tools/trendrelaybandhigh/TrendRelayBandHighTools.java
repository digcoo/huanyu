...
public final class TrendRelayBandHighTools {
    ...
    private TrendRelayBandHighTools() {
    ...
    public static Hit findHit(StockBase stock, TrendRelayBandHighStrategyParams params) {
    ...
    public static String buildTrendMessage(Hit hit) {
        ...
        if (hit == null) {
            ...
            return "[TRBH]趋势中转破末High";

        }

        return String.format("[TRBH]趋势中转破末High|%sMACD>0,未回踩Low,末K首次破bandHigh|bandHigh=%.2f",
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                ...
                "趋势中转破末High,strategyTag=TRBH,period=%s,sigDay=%s,sigClose=%.2f,bandHigh=%.2f,bandLow=%.2f",
    ...
    public static PeriodTypeEnum resolvePeriod(TrendRelayBandHighStrategyParams.Tier tier) {

        if (tier == TrendRelayBandHighStrategyParams.Tier.WEEK) {

            return PeriodTypeEnum.WEEK;

        }

        if (tier == TrendRelayBandHighStrategyParams.Tier.MONTH) {
    ...
    public static int resolveLookback(TrendRelayBandHighStrategyParams params) {

        TrendRelayBandHighStrategyParams p = params != null ? params : TrendRelayBandHighStrategyParams.defaults();

        if (p.getTier() == TrendRelayBandHighStrategyParams.Tier.WEEK) {

            return p.getLookbackWeek();

        }

        if (p.getTier() == TrendRelayBandHighStrategyParams.Tier.MONTH) {
