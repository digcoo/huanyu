...
public final class CascadeWaveConcaveDayScoreCalculator {

    private CascadeWaveConcaveDayScoreCalculator() {
    }

    public static int computeScore(CascadeWaveConcaveDayEvaluator.CascadeWaveConcaveDayEvaluation eval) {
        int score = 34;
        int hits = 0;
    ...
    public static PeriodTypeEnum primaryPeriod(CascadeWaveConcaveDayEvaluator.CascadeWaveConcaveDayEvaluation eval) {
        CascadeWaveBreakoutTools.TierHit hit = primaryHit(eval);
        return hit != null ? hit.getSignalTier() : PeriodTypeEnum.DAY;
    }

    public static String buildTrendMessage(CascadeWaveConcaveDayEvaluator.CascadeWaveConcaveDayEvaluation eval,
                                           CascadeWaveConcaveDayStrategyParams params) {
        StringBuilder sb = new StringBuilder("[CASCADEWAVECONCAVEDAY]级联MACD凹波段日突破");
    ...
    public static String buildSignalMessage(CascadeWaveBreakoutTools.TierHit hit) {
        ...
        return String.format(
                "级联MACD凹波段日突破,bandShape=CONCAVE,edgeMode=DAY,breakPath=%s,signalTier=%s,refPeriod=day,"
    ...
    private static void appendTierTrendLabel(StringBuilder sb, CascadeWaveBreakoutTools.TierHit hit) {
        if (hit != null) {
            sb.append('|').append(periodLabel(hit.getSignalTier())).append("级联凹日突破");
        }
    }
