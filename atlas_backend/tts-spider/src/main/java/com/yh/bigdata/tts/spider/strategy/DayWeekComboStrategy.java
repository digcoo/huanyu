...
import com.yh.bigdata.tts.spider.strategy.tools.dayweek.DayWeekComboEvaluator;
...
@Slf4j
@Component
public class DayWeekComboStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.DAY_WEEK_COMBO;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            DayWeekComboStrategyParams params = resolveParams(queryContextParam);
            DayWeekComboEvaluator.DayWeekComboEvaluation eval =
                    DayWeekComboEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private DayWeekComboStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getDayWeekCombo() == null) {
            return DayWeekComboStrategyParams.defaults();
        }
        return DayWeekComboStrategyParams.merge(queryContextParam.getDayWeekCombo());
    }
