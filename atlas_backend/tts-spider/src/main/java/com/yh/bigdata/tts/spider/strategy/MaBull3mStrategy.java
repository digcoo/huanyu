...
import com.yh.bigdata.tts.spider.strategy.tools.mabull3m.MaBull3mEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.mabull3m.MaBull3mTools;
...
@Slf4j
@Component
public class MaBull3mStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_BULL_3M;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            MaBull3mStrategyParams params = resolveParams(queryContextParam);
            MaBull3mEvaluator.MaBull3mEvaluation eval =
                    MaBull3mEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : MaBull3mTools.resolvePeriod(params.getTier());
    ...
    private MaBull3mStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaBull3m() == null) {
            return MaBull3mStrategyParams.defaults();
        }
        return MaBull3mStrategyParams.merge(queryContextParam.getMaBull3m());
    }
