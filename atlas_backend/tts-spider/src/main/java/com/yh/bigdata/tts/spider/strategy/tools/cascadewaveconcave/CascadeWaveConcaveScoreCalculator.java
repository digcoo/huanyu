...
public final class CascadeWaveConcaveScoreCalculator {

    private CascadeWaveConcaveScoreCalculator() {
    }

    public static int computeScore(CascadeWaveConcaveEvaluator.CascadeWaveConcaveEvaluation eval) {
        int score = 34;
        int hits = 0;
    ...
    public static PeriodTypeEnum primaryPeriod(CascadeWaveConcaveEvaluator.CascadeWaveConcaveEvaluation eval) {
        CascadeWaveBreakoutTools.TierHit hit = primaryHit(eval);
        return hit != null ? hit.getSignalTier() : PeriodTypeEnum.DAY;
    }

    public static String buildTrendMessage(CascadeWaveConcaveEvaluator.CascadeWaveConcaveEvaluation eval,
                                           CascadeWaveConcaveStrategyParams params) {
        StringBuilder sb = new StringBuilder("[CASCADEWAVECONCAVE]级联MACD凹波段同档突破");
    ...
    public static String buildSignalMessage(CascadeWaveBreakoutTools.TierHit hit) {
        ...
        return String.format(
                "级联MACD凹波段突破,bandShape=CONCAVE,breakPath=%s,signalTier=%s,refPeriod=%s,"
    ...
    private static void appendTierTrendLabel(StringBuilder sb, CascadeWaveBreakoutTools.TierHit hit) {
        if (hit != null) {
            sb.append('|').append(periodLabel(hit.getSignalTier())).append("级联凹同档突破");
        }
    }
