...
import com.yh.bigdata.tts.spider.strategy.tools.waveconcaveday.WaveConcaveDayEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.waveconcaveday.WaveConcaveDayScoreCalculator;
...
@Slf4j
@Component
public class WaveConcaveDayStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.WAVE_CONCAVE_DAY;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            WaveConcaveDayStrategyParams params = resolveParams(queryContextParam);
            WaveConcaveDayEvaluator.WaveConcaveDayEvaluation eval =
                    WaveConcaveDayEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            ...
            checkResult.setOpPeriodType(WaveConcaveDayScoreCalculator.primaryPeriod(eval));
    ...
    private WaveConcaveDayStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getWaveConcaveDay() == null) {
            return WaveConcaveDayStrategyParams.defaults();
        }
        return WaveConcaveDayStrategyParams.merge(queryContextParam.getWaveConcaveDay());
    }
