...
import com.yh.bigdata.tts.spider.strategy.tools.trendretestlow.TrendRetestLowEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.trendretestlow.TrendRetestLowTools;
...
@Slf4j
@Component
public class TrendRetestLowStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.TREND_RETEST_LOW;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            TrendRetestLowStrategyParams params = resolveParams(queryContextParam);
            TrendRetestLowEvaluator.TrendRetestLowEvaluation eval =
                    TrendRetestLowEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : TrendRetestLowTools.resolvePeriod(params.getTier());
    ...
    private TrendRetestLowStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getTrendRetestLow() == null) {
            return TrendRetestLowStrategyParams.defaults();
        }
        return TrendRetestLowStrategyParams.merge(queryContextParam.getTrendRetestLow());
    }
