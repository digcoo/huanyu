package com.yh.bigdata.tts.spider.strategy.tools.trendrelaybreakhigh;

import com.yh.bigdata.tts.common.constants.PeriodTypeEnum;
import com.yh.bigdata.tts.common.constants.RealtimeStockCache;
import com.yh.bigdata.tts.common.model.StockBase;
import com.yh.bigdata.tts.common.model.Trade;
import com.yh.bigdata.tts.common.param.TrendRelayBreakHighStrategyParams;
...
/**
 * 末波段后未回踩 bandLow，且末 K 首次边沿突破末波段 bandHigh。
 */
public final class TrendRelayBreakHighTools {

    private TrendRelayBreakHighTools() {
    }
    ...
    public static Hit findHit(StockBase stock, TrendRelayBreakHighStrategyParams params) {
        TrendRelayBreakHighStrategyParams p = params != null ? params : TrendRelayBreakHighStrategyParams.defaults();
    ...
    static Hit findHitOnBars(List<Trade> trades, PeriodTypeEnum period, int lookback) {
        ...
        for (int i = window.getTerminatorIdx() + 1; i < lastIdx; i++) {
            ...
            if (bar != null && bar.getClose() != null && bar.getClose() > bandHigh + 1e-6) {
                return null;
            }
        }
    ...
    public static boolean passesOptionalGates(StockBase stock, CheckResult checkResult,
                                              TrendRelayBreakHighStrategyParams params) {
        TrendRelayBreakHighStrategyParams p = params != null ? params : TrendRelayBreakHighStrategyParams.defaults();
    ...
    public static String buildTrendMessage(Hit hit) {
        if (hit == null) {
            return "[TRBH]趋势中转破High";
        }
        return String.format("[TRBH]趋势中转破High|%sMACD>0,未回踩Low,末K首次破High|bandHigh=%.2f",
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                "趋势中转破High,strategyTag=TRBH,period=%s,sigDay=%s,sigClose=%.2f,bandHigh=%.2f,bandLow=%.2f",
    ...
    public static PeriodTypeEnum resolvePeriod(TrendRelayBreakHighStrategyParams.Tier tier) {
        if (tier == TrendRelayBreakHighStrategyParams.Tier.WEEK) {
            return PeriodTypeEnum.WEEK;
        }
        if (tier == TrendRelayBreakHighStrategyParams.Tier.MONTH) {
            return PeriodTypeEnum.MONTH;
        }
        return PeriodTypeEnum.DAY;
    }

    public static int resolveLookback(TrendRelayBreakHighStrategyParams params) {
        TrendRelayBreakHighStrategyParams p = params != null ? params : TrendRelayBreakHighStrategyParams.defaults();
        if (p.getTier() == TrendRelayBreakHighStrategyParams.Tier.WEEK) {
            return p.getLookbackWeek();
        }
        if (p.getTier() == TrendRelayBreakHighStrategyParams.Tier.MONTH) {
    ...
    private static void appendMessage(CheckResult checkResult, PeriodTypeEnum period, String msg) {
        if (checkResult != null && period != null) {
            checkResult.addTrendPeriod(period, "[TRBH]" + msg);
        }
    }
}
