...
/**
 * 均线多头突破：MA10&gt;MA20&gt;MA30，末 K close&gt;均线MAX，
 * 且（边沿突破均线MAX 或 开盘突破均线MAX）。
 */
public final class MaAlignLiftTools {

    private static final double EPS = 1e-6;

    private MaAlignLiftTools() {
    }
    ...
    public static Hit findHit(StockBase stock, MaAlignLiftStrategyParams params) {
    ...
    static Hit findHitOnBars(List<Trade> trades, PeriodTypeEnum period) {
        ...
        if (!passesMaBullOrder(signalBar)) {
            return null;
        }
    ...
    /** 文档：MA10 &gt; MA20 &gt; MA30（末 K） */
    static boolean passesMaBullOrder(Trade bar) {
        if (bar == null) {
            return false;
        }
        ...
        return ma10 > ma20 + EPS && ma20 > ma30 + EPS;
    }
    ...
    public static String buildTrendMessage(Hit hit) {
        ...
        return String.format("[MAL]均线多头突破|%sMA10>MA20>MA30,末K>均线MAX,%s|maxMA=%.2f",
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                "均线多头突破,strategyTag=MAL,period=%s,sigDay=%s,sigClose=%.2f,maxMA=%.2f,breakout=%s",
    ...
    public static PeriodTypeEnum resolvePeriod(MaAlignLiftStrategyParams.Tier tier) {
        if (tier == MaAlignLiftStrategyParams.Tier.WEEK) {
            return PeriodTypeEnum.WEEK;
        }
        if (tier == MaAlignLiftStrategyParams.Tier.MONTH) {
    ...
    private static void appendMessage(CheckResult checkResult, PeriodTypeEnum period, String msg) {
        if (checkResult != null && period != null) {
            checkResult.addTrendPeriod(period, "[MAL]" + msg);
        }
    }
}
