...
/**
 * 日周组合：周+日 MACD&gt;0、周 close&gt;近2周 low 高值；日金叉波段 High 边沿突破（信号限末自然周）。
 */
public final class DayWeekComboTools {

    private static final double EPS = 1e-6;
    private static final int WEEK_MACD_LOOKBACK = 40;
    private static final int DAY_MACD_LOOKBACK = 40;

    private DayWeekComboTools() {
    }
    ...
    public static Hit findHit(StockBase stock, DayWeekComboStrategyParams params) {
        DayWeekComboStrategyParams p = params != null ? params : DayWeekComboStrategyParams.defaults();
        if (stock == null) {
            return null;
        }
        if (!passesWeekDayGate(stock)) {
            return null;
        }
        int fetchBars = Math.max(120, (p.getPrevWeeks() + 1) * p.getMaxBarsPerWeek() + p.getGcLookbackBars() + 20);
    ...
    public static boolean passesWeekDayGate(StockBase stock) {
        ...
        if (!passesWeekCloseAboveRecentLows(weekBars)) {
            return false;
        }
    ...
    public static String buildSignalMessage(Hit hit) {
        ...
        return String.format(
                "日周组合,strategyTag=DW,period=day,gcDay=%s,gcClose=%.2f,"
    ...
    public static String buildTrendMessage(Hit hit) {
        ...
        return String.format("[DW]日周组合|周+日MACD>0,日破波段High|bandHigh=%.2f",
    ...
    private static void appendMessage(CheckResult checkResult, String msg) {
        if (checkResult != null) {
            checkResult.addTrendPeriod(PeriodTypeEnum.DAY, "[DW]" + msg);
        }
    }
}
