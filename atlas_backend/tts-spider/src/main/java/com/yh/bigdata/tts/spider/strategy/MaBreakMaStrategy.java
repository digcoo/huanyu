...
import com.yh.bigdata.tts.spider.strategy.tools.mabreakma.MaBreakMaEvaluator;
...
@Slf4j
@Component
public class MaBreakMaStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.MA_BREAK_MA;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            MaBreakMaStrategyParams params = resolveParams(queryContextParam);
            MaBreakMaEvaluator.MaBreakMaEvaluation eval =
                    MaBreakMaEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primary = eval.getPeriod() != null
                    ? eval.getPeriod()
                    : MaBreakMaTools.resolvePeriod(params.getTier());
    ...
    private MaBreakMaStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getMaBreakMa() == null) {
            return MaBreakMaStrategyParams.defaults();
        }
        return MaBreakMaStrategyParams.merge(queryContextParam.getMaBreakMa());
    }
