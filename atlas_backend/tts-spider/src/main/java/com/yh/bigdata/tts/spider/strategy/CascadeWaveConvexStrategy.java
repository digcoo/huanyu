...
import com.yh.bigdata.tts.spider.strategy.tools.cascadewaveconvex.CascadeWaveConvexEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.cascadewaveconvex.CascadeWaveConvexScoreCalculator;
...
@Slf4j
@Component
public class CascadeWaveConvexStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.CASCADE_WAVE_CONVEX;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            CascadeWaveConvexStrategyParams params = resolveParams(queryContextParam);
            CascadeWaveConvexEvaluator.CascadeWaveConvexEvaluation eval =
                    CascadeWaveConvexEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            ...
            checkResult.setOpPeriodType(CascadeWaveConvexScoreCalculator.primaryPeriod(eval));
    ...
    private CascadeWaveConvexStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getCascadeWaveConvex() == null) {
            return CascadeWaveConvexStrategyParams.defaults();
        }
        return CascadeWaveConvexStrategyParams.merge(queryContextParam.getCascadeWaveConvex());
    }
