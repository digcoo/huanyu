...
import com.yh.bigdata.tts.spider.strategy.tools.daymin60.DayMin60ComboEvaluator;
...
@Slf4j
@Component
public class DayMin60ComboStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.DAY_MIN60_COMBO;
    }
    ...
    public List<PeriodTypeEnum> getTrendPeriodTypes() {
        return Arrays.asList(PeriodTypeEnum.DAY, PeriodTypeEnum.MIN60);
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            DayMin60ComboStrategyParams params = resolveParams(queryContextParam);
            DayMin60ComboEvaluator.DayMin60ComboEvaluation eval =
                    DayMin60ComboEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
    ...
    private DayMin60ComboStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getDayMin60Combo() == null) {
            return DayMin60ComboStrategyParams.defaults();
        }
        return DayMin60ComboStrategyParams.merge(queryContextParam.getDayMin60Combo());
    }
