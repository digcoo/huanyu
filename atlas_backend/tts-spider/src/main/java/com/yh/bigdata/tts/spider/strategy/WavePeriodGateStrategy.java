...
import com.yh.bigdata.tts.spider.strategy.tools.waveperiodgate.WavePeriodGateEvaluator;
import com.yh.bigdata.tts.spider.strategy.tools.waveperiodgate.WavePeriodGateTools;
...
@Slf4j
@Component
public class WavePeriodGateStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.WAVE_PERIOD_GATE;
    }
    ...
    public CheckResult check(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes,
        ...
        try {
            WavePeriodGateStrategyParams params = resolveParams(queryContextParam);
            WavePeriodGateEvaluator.WavePeriodGateEvaluation eval =
                    WavePeriodGateEvaluator.evaluate(stockBase, checkResult, params);
            if (!eval.isHit()) {
                return checkResult;
            }
            PeriodTypeEnum primaryPeriod = WavePeriodGateTools.resolvePeriod(params.getTier());
            ...
            checkResult.addTrendPeriod(primaryPeriod, "[WPG]形态门|" + tierLabel);
            checkResult.addSignal(primaryPeriod, "形态门,strategyTag=WPG,tier=" + tierLabel);
        } catch (Exception ex) {
    ...
    private WavePeriodGateStrategyParams resolveParams(QueryContextParam queryContextParam) {
        if (queryContextParam == null || queryContextParam.getWavePeriodGate() == null) {
            return WavePeriodGateStrategyParams.defaults();
        }
        return WavePeriodGateStrategyParams.merge(queryContextParam.getWavePeriodGate());
    }

    static String buildTierLabel(WavePeriodGateStrategyParams params) {
