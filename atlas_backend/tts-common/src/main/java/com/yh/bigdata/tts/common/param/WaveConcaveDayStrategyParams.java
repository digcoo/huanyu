...
/**
 * 凹波段日突破（waveconcaveday）· 末阳顶默认可选末阳中位 · 日 K 边沿
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class WaveConcaveDayStrategyParams {
    ...
    public static WaveConcaveDayStrategyParams defaults() {
        return WaveConcaveDayStrategyParams.builder().build();
    }

    public static WaveConcaveDayStrategyParams merge(WaveConcaveDayStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        WaveConcaveDayStrategyParams d = defaults();
