...
/**
 * 趋势回踩破 Low（trendretestlow）：MACD&gt;0 + 回踩 bandLow + 末 K 收阳且 bandLow&lt;close≤bandHigh。
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class TrendRetestLowStrategyParams {
    ...
    public static TrendRetestLowStrategyParams defaults() {
        return TrendRetestLowStrategyParams.builder().build();
    }

    public static TrendRetestLowStrategyParams merge(TrendRetestLowStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        TrendRetestLowStrategyParams d = defaults();
