...
/**
 * 趋势回踩破 High（trendretesthigh）：MACD&gt;0 + 回踩 bandLow + 末 K 首次边沿突破 bandHigh。
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class TrendRetestHighStrategyParams {
    ...
    public static TrendRetestHighStrategyParams defaults() {
        return TrendRetestHighStrategyParams.builder().build();
    }

    public static TrendRetestHighStrategyParams merge(TrendRetestHighStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        TrendRetestHighStrategyParams d = defaults();
