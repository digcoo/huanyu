...
/**
 * 凸波段突破（waveconvex）· 末阳中位价默认可选末阳顶 · 同档边沿
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class WaveConvexStrategyParams {
    ...
    public static WaveConvexStrategyParams defaults() {
        return WaveConvexStrategyParams.builder().build();
    }

    public static WaveConvexStrategyParams merge(WaveConvexStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        WaveConvexStrategyParams d = defaults();
