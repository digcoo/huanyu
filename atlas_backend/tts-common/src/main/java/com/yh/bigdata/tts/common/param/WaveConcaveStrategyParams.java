...
/**
 * 凹波段突破（waveconcave）· 末阳顶默认可选末阳中位 · 同档边沿
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class WaveConcaveStrategyParams {
    ...
    public static WaveConcaveStrategyParams defaults() {
        return WaveConcaveStrategyParams.builder().build();
    }

    public static WaveConcaveStrategyParams merge(WaveConcaveStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        WaveConcaveStrategyParams d = defaults();
