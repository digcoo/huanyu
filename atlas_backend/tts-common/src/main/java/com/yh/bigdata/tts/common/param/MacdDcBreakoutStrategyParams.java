...
/**
 * MACD死叉突破（macddcb）：MACD&lt;0、同档边沿突破死叉K前一根K high。
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class MacdDcBreakoutStrategyParams {
    ...
    public static MacdDcBreakoutStrategyParams defaults() {
        return MacdDcBreakoutStrategyParams.builder().build();
    }

    public static MacdDcBreakoutStrategyParams merge(MacdDcBreakoutStrategyParams incoming) {
        if (incoming == null) {
            return defaults();
        }
        MacdDcBreakoutStrategyParams d = defaults();
