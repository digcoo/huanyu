...
@Slf4j
public final class BandIndicatorUtils {
    ...
    public static List<BandSegment> getBandSegmentListWithoutCurrent(PeriodTypeEnum trendPeriodType,List<Trade> trades) {
        ...
        targetList.removeIf(x -> MathUtil.max(x.getBandShiTiRate(), x.getBandChangeRate()) < trendPeriodType.getBandShiTiRate());
...
//                    || (trades.get(j-1).getChangeRate() <= 0 && (trades.get(j).getChangeRate() > 0 || trades.get(j).getShitiRate() >= 0)))) {
            ...
            // 过滤太小的波段
            BandSegment newBandSegment = new BandSegment(bandStart, bandEnd, high, low, new ArrayList<>(rangeTrades), preBandStart);
            bandSegments.add(newBandSegment);
...
//        private PeriodTypeEnum periodType;
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Trade> dayTrades = XueQiuUtils.getXueQiuJson("sh600844", trendPeriodType.getCode());
...
//        }
        ...
        List<BandSegment> bandSegmentListWithoutCurrent = BandIndicatorUtils.getBandSegmentListWithoutCurrent(trendPeriodType, dayTrades);
...
//            log.info("trade : {} - {}", trade.getDay(), trade.getClose());
//        }
//
//        BandSegment lastBandSegmentWithoutCurrent0 = BandIndicatorUtils.getLastBandSegmentWithoutCurrent(trendPeriodType, dayTrades, 0);
