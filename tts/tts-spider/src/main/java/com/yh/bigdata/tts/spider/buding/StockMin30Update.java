...
@Component
public class StockMin30Update {

	Logger logger = LoggerFactory.getLogger(StockMin30Update.class);
			...
			while (true) {
				...
				if (!CollectionUtils.isEmpty(pages.getResult())) {
					for (StockBase stockBase : pages) {
                        List<StockMin30> stockDays = stockMin30Mapper.selectAll(Arrays.asList(stockBase.getCode()));

                        Map<String, Trade> maMap = MAIndicatorUtils.calAllMAsAndFill(stockBase, stockDays);
                        for (StockMin30 stockDay : stockDays) {
                            ...
                            if (Objects.nonNull(trade)) {
                                ...
                                stockMin30Mapper.updateByPrimaryKey(stockDay);
                            }
