...
@Slf4j
@Component
public class CrossBandStepHighRealtimeStrategy extends AbstractStrategy {

    @Override
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.CROSS_BAND_STEP_HIGH_REALTIME;
    }
    ...
    public boolean checkSignal(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes, PeriodTypeEnum opPeriodType, CheckResult checkResult, QueryContextParam queryContextParam) {
        ...
        // 突破趋势波段step: high
        boolean checkCrossBandHighSignal = SignalMin30ToolsWrapper.checkCrossBandStepSignal(stockBase, trendPeriodTypes, opPeriodType, checkResult,true);
        if (checkCrossBandHighSignal) {
            return true;
        }

        // 突破反转波段step: high
        boolean checkRevertBandHighSignal = SignalMin30ToolsWrapper.checkRevertBandStepSignal(stockBase, trendPeriodTypes, opPeriodType, checkResult, true);
