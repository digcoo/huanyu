...
public class CrossBandPressureStrategy extends AbstractStrategy {
    ...
    public StrategyTypeEnum getStrategy() {
        return StrategyTypeEnum.CROSS_BAND_PRESSURE;
    }
    ...
    public boolean checkSignal(StockBase stockBase, List<PeriodTypeEnum> trendPeriodTypes, PeriodTypeEnum opPeriodType, CheckResult checkResult, QueryContextParam queryContextParam) {
        ...
        // 突破趋势波段
        boolean checkCrossBandHighSignal = SignalBandToolsWrapper.checkCrossBandHighSignal(stockBase, trendPeriodTypes, opPeriodType, checkResult);
        if (checkCrossBandHighSignal) {
            return true;
        }

        // 突破反转波段
        boolean checkRevertBandHighSignal = SignalBandToolsWrapper.checkRevertBandHighSignal(stockBase, trendPeriodTypes, opPeriodType, checkResult);
